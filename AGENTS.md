...
- `apps/bot/src/bot.ts`: main bot entrypoint, Telegram command registration, Express server bootstrap, scheduled HTTP routes, OAuth callback handling, and API route registration.
- `apps/bot/src/platform.ts`: shared bot bootstrap, `.env` loading, `grammy` session/conversation setup, and Supabase client initialization.
...
- `apps/bot/src/google.ts`: Google OAuth, token refresh, People API sync, cooldown enforcement, and Telegram sync report formatting.
- `apps/bot/src/commands/`: Telegram command handlers and conversation flows.
...
- If a task touches birthday reminders, verify both the Telegram command path and the scheduled request path because the same backend owns both.
